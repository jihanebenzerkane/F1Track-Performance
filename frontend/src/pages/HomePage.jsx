...
function AnimatedCar({ modelPath }) {
  ...
  // Auto-center the model exactly like CarViewer
  useMemo(() => {
    ...
    // Kept perfectly in the center (removed vertical bumps)
    scene.rotation.y = Math.PI / 2; // Test rotation if it spawns sideways
...
export default function HomePage() {
  ...
  return (
    ...
    <div style={{
      ...
      {/* Dark Gradient Overlay near top and bottom for readability */}
      <div style={{
        position: 'fixed',
        inset: 0,
        zIndex: 1,
...
{ label: '2026 REGULATION', value: 'Active aero — no DRS' },
        ...
        <div style={{
          ...
          <button onClick={() => navigate('/standings?season=2025')} className="home-btn" style={{
            ...
            color: '#000000ff', border: 'none', borderRadius: '12px',background:'#ffffff71',
          ...
          <button onClick={() => navigate('/predictions')} className="home-btn" style={{
            padding: '24px', fontSize: '15px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '2px',
            color: '#000', background: '#ffffff70', border: 'none', borderRadius: '12px',
            cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 8px 32px rgba(255,255,255,0.15)',
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px'
          }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-4px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
